...
> [!info]- 泊松分布的实际意义
> 泊松分布可以由二项分布推导而来，可以用于描述单位时间内事件的发生次数。考虑一个在任何时刻都可能发生的事件 $A$ ，我们想要计算 $A$ 在单位时间内发生次数 $X$ 的分布。  
> 如果我们将单位时间 1 划分为 $n$ 的尽量小的等长时间片，那么每个时间片内可以近似认为 $A$ 以概率 $p$ 只发生一次，那么 $n$ 个时间片中发生了 $A$ 次数 $k$ 次服从二项分布
> $$
X\sim B(n,p),\quad P\{ X=k \}=C_{n}^{k}p^{k}(1-p)^{n-k}
> $$
> 为了使得切片的误差是一个可以忽略的无穷小量，我们对上式取极限，即令 $n\to \infty$，此时 $p\to 0$，于是得到泊松分布
> $$
\lim\limits_{ n \to \infty,p\to 0 } C_{n}^{k}p^{k}(1-p)^{n-k}=\lim\limits_{ n \to \infty, p\to 0 } \frac{n!}{(n-k)!k!}p^{k}(1-p)^{n-k}
> $$
> 设 $\lambda=np$，当 $n\to \infty,p\to 0$ 时 $\lambda$ 是一个未定式。如果 $n$ 足够大，$p$ 足够小，$\lambda$ 保持一个合适的值的时候 (即 $\lim\limits_{ n \to \infty }\lambda(n)=\lambda$)，上述极限可以转换为
> $$
\lim\limits_{ n \to \infty } \frac{n(n-1)\cdots(n-k+1)}{n^{k}}\frac{\lambda^{k}}{n^{k}}\frac{(1-n)}{}
> $$
> 
> **单位时间**内源源不断的质点来流的个数，也常用于描述稀有事件发生的次数。其中 $\lambda$ 叫做来流强度，也是数学泊松分布的数学期望。
> 
> > [!info|noicon] 泊松过程
> > 泊松分布表明了在单位时间内源源不断到来的质点的个数，而泊松过程则表明了 $t$ 时刻位置不断到来的质点个数，显然，当 $t=t_{0}$ 时，泊松过程对应一个泊松分布 $P(\lambda t_{0})$。
...
> [!info]- 指数分布是连续型等待分布
> 指数分布是泊松分布的推导而来，下面计随机过程 $\{ N_{t},t\geqslant 0 \}$，其中 $N_{t}$ 表示到时间 $t$ 为止到来的质点数量。
