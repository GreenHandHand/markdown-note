...
之前讨论的所有评价指标都是针对单类别的任务。现实中，存在另一种分类任务，称为多标签分类任务 (multi-label classification)。在多标签分类任务中，每个样本都可以联系到一个以上的类别。例如，给定一张图片，预测其中的物体。

对于多标签分类任务，使用的评价指标将会有一些变化。这里了解一些常用的指标。

### Precision as k

precision at k or P@K 定义为预测结果中概率最大的 k 个标签预测正确的概率。