...
pandas 提供了高级数据结构和函数，这些数据结构和函数的设计使得利用结构化、表格化数据的工作快速、简单、有表现力。该部分内容是后续 [[pandas数据载入与读取]]、[[pandas数据类型与修改]]、[[pandas数据清洗与准备]]、[[pandas数据规整：连接、联合与重塑]]、[[pandas数据聚合与分组操作]] 的前置知识。
