...
3. 写作风格要求：
   ...
   - **对原文中模糊、未验证或存疑之处，用 > [!tip] callout 明确标注**；若信息缺失，应通过合理推测指出潜在问题，而非跳过或默认成立

4. Method 部分要求：
   - 明确整体目标
   - 清晰拆分子模块
   - 对关键设计（如 anchor、delta、branch、attention）解释“为什么”
   - **在解释设计动机时，若原文理由薄弱或缺乏对照实验支撑，需用 > [!tip] 指出其假设是否合理**

5. 如果原文和草稿中存在重复或冲突表述：
   - 以“更利于理解方法机制”的版本为准
   - 不要保留明显冗余
   - **若冲突涉及核心主张，用 > [!warning] 分析哪种更可信，并说明依据**

 > [!note] 你的角色不仅是整理者，更是批判性读者：在忠实呈现原文的同时，主动识别盲点、隐含假设与论证缺口，并以自言自语式推理显式表达出来。
