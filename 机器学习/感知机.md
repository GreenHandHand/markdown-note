...
感知机 (perceptron) 是二分类的线性分类模型，其输入为实例的特征向量，输出为实例的类别，取 +1 与 -1 二值。感知机对应将输入空间中实例划分为正负两类的线性超平面，属于判别模型。感知机 1957 年由 Rosenblatt 提出，是神经网络与支持向量机的基础。
...
此外，对于一些特殊的状态，例如文本分词中的前一个字符是否是数字、当前字符是否等于前一个字符等，需要根据当前的任务进行特殊的设计。这样的特征使用
$$
\phi_l(x_t,y_t)=\begin{cases}1\\0\end{cases}
$$