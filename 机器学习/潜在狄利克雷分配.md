...
多项分布 (multinomial distribution) 是一种多元离散随机变量的概率分布，是二项分布 (binomial distribution) 的扩展。其定义如下：若多元离散随机变量 $X=(X_1,X_2,\cdots,X_k)$ 的概率质量函数为
$$
\begin{aligned}
...
一个有限的随机变量序列是可交换的 (exchangeable) 是指随机变量的联合概率分布对随机变量的排列不变，即
$$
