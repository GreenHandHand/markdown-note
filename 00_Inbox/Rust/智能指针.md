...
**指针** (*pointer*) 是一个包含内存地址的变量的通用概念。这个地址引用，或 “指向”（points at）一些其它数据。Rust 中最常见的指针 [[00_Inbox/Rust/所有权#引用|引用]] (*reference*)。引用以 `&` 符号为标志并借用了它们所指向的值。除了引用数据没有任何其他特殊功能，也没有额外开销。
...
> [!note] `Rc<T>` 只能用于单线程的情况，因为它对引用计数的操作不是 [[02_Areas/操作系统/进程管理#^96b86a|原子操作]]。
...
> [!warning]
> 通过不可变引用， `Rc<T>` 允许在程序的多个部分之间只读地共享数据。如果 `Rc<T>` 也允许多个可变引用，则会违反 [[00_Inbox/Rust/所有权#引用|借用规则]] 之一：相同位置的多个可变借用可能造成数据竞争和不一致。
>
...
---
< [[00_Inbox/Rust/Cargo|Cargo]] | [[00_Inbox/Rust/无畏并发|无畏并发]] >
