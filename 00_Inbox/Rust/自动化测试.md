...
> [!note] 集成测试中的子模块
> 当我们在集成测试中想要创建一个用于多个测试的公用代码时 (例如初始化环境)，我们无法使用在 [[00_Inbox/Rust/包与模块|包与模块]] 中的方式创建一个同名的文件，因为在 Rust 中所有 `tests` 目录下的文件都会视为一个 crate 进行测试，即使文件中没有任何的测试函数。
>
...
---
< [[00_Inbox/Rust/泛型、Trait与生命周期|泛型、Trait与生命周期]] | [[00_Inbox/Rust/函数式特性|函数式特性]] >
