...
> 的函数都天然是半正定核。证明如下。
> 
> 对于任意样本 $x_{1},x_{2},\cdots,x_{n}$ 和任意系数 $c_{1},c_{2},\cdots,c_{n}$ 有
> $$
> \sum\limits_{i,j}c_{i}c_{j}\kappa(x_{i},x_{j})=\sum\limits_{i,j}c_{i}c_{j}\phi(x_{i})^{\top}\phi(x_{j})=\left\lVert  \sum\limits_{i}c_{i}\phi(x_{i})  \right\rVert ^{2} \geqslant 0
> $$
