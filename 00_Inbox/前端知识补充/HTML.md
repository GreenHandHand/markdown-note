...
style 属性用于改变 HTML 元素的样式。style 属性通过 [[00_Inbox/前端知识补充/CSS]] 样式来调整文字与段落的属性。由于引入了 CSS 样式，因此一些曾经使用的标签与属性应当避免使用，如下表所示：
...
使用 `<script>code</script>` 来定义一个 [[00_Inbox/前端知识补充/JavaScript]] 脚本。网页可以通过脚本来动态的进行改变，使用 `<nonscript></nonscript>` 来指定当 JavaScript 不可用时显示的内容。
...
使用 `<div></div>` 与 `<table></table>` 来进行网页的布局。通常使用 `<div>` 与 [[00_Inbox/前端知识补充/CSS|CSS]] 可以轻松的实现网页的布局。`<table>` 不是针对网页布局设计的，但是仍然可以用于网页的布局。
