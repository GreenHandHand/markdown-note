...
> [!note] 前馈神经网络的局限性
> 在前馈神经网络中，信息是单向传递的，这种限制虽然使得网络变得更容易学习，但在一定程度上也减弱了神经
### 统计工具
...
$$
相比之下，该网络结构增加了 $H_{t-1}W_{hh}$ 项，该项保留了历史信息，相当于神经网络的记忆，因此这样的变量称为隐状态。由于当前状态中隐状态的计算方式与上一状态时相同，因此上式的计算是循环的 (recurrent) ，我们将该神经网络称为循环神经网络 (recurrent nerual network)，在循环神经网络中执行计算的层被称为循环层 (recurrent layer)。
...
最后，我们可以使用一个随机变量替换 $\partial h_t/\partial w_h$。该随机变量在预测中是正确的，但是会截断序列。这个随机变量是通过序列 $\xi_t$ 来实现的，序列预定义了 $0\leqslant \pi\leqslant 1$，其中 $P(\xi_t=0)=1-\pi_t$ 且 $P(\xi_t=\pi_t^{-1})=\pi_t$，因此 $E[\xi_t]=1$。我们使用它来替换 $\partial h_t/\partial w_h$，得到
$$
