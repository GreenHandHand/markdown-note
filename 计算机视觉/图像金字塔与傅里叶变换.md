...
连续情况傅里叶变换、逆变换：
$$
F(k)=\int_\infty^{-\infty}f(x)e^{-\omega jx}\mathrm dx \Leftrightarrow f(x)=\int_{\infty}^{-\infty}F(k)e^{\omega jx}dk
$$
离散傅里叶变换、逆变换：
$$
F(k)=\frac{1}{N}\sum_{x=0}^{N-1}f(x)e^{-\omega jx/N}\Leftrightarrow f(x)=\sum_{k=0}^{N-1}F(k)e^{\omega jx/N}
$$
其中，$\omega=2\pi f$ 是圆频率。
