...
为了实现尺度不变性，我们希望可以在不同的区域中取到有关这个函数的相同的值，并将这个尺度作为特征的窗口尺度。因此，如果这个函数是单峰的，那么它的最值点一定唯一，我们可以将这个最值点作为尺度。于是，我们希望找到一个函数，它满足：
...
- Laplacian 算子：$L=\sigma^2(G_{xx}(x,y,\sigma)+G_{yy}(x,y,\sigma))$
- 高斯差分算子 (Difference of Gaussians, DoG)：$DoG=G(x,y,k\sigma)-G(x,y,\sigma)$，其中 $G(x,y,\sigma)=\frac{1}{\sqrt{2\pi}\sigma}e^{-\frac{x^2+y^2}{2\sigma^2}}$

它们都具有尺度不变性和旋转不变性，有类似的图像：
