...
在 Harris 算法中，每个像素点对应了一个移动窗口。为了得到角点，我们需要一个函数来计算局部窗口中亮度的变化，角点就是该函数的极大值。

设像素点 $(x,y)$，则以其为中心的移动窗口函数为 $w(x,y)$。$I(x,y)$ 表示原图像的亮度值，将其平移 $(u,v)$ 个单位后的亮度值为 $I(x+u,y+v)$，于是
$$
E(u,v)=\sum_{x,y}w(x,y)[I(x+u,y+v)-I(x,y)]^2
$$
其中 $w$ 是窗口函数，可以是一个线性滤波器，一般使用高斯滤波器。为了计算这个函数的极大值，我们对其进行一些处理。首先，将 $I(x+u,y+v)$ 在 $(x,y)$ 处进行泰勒展开，可以得到：
$$
